...
for generation = 1:numOfGenerations
    ...
    [mse, isBest, index] = totalFitness(chromosomes, n, u1, u2, gauss,...
                            threshold, numOfGaussians, f);
    if isBest == true
        plotApproximated(index, numOfGaussians, chromosomes, gauss);
        break;
    end
...
plotApproximated(1, numOfGaussians, chromosomes, gauss);
...
function [mse, isBest, index] = totalFitness(chromosomes, n, u1, u2, gauss,...
    ...
    for chromo = 1:n
        res = 0;
        for i = 1:numOfGaussians
            res = res - 0.1 + gauss(u1, u2, chromosomes(chromo, i, 1),...
...
function plotApproximated(index, numOfGaussians, chromosomes, gauss)
    [X, Y] = meshgrid(-1:0.05:2, -2:0.05:1);
    res = 0;
    for i = 1:numOfGaussians
        res = res - 0.1 + gauss(X, Y, chromosomes(index, i, 1),...
